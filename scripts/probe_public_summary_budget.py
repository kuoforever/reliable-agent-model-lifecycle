"""One doubled-budget reference-text summary; no browser or desktop interface."""
...
def completion_metrics(input_tokens, tokens, eos_ids, seconds, peak, output_bytes):
    ...
    value = dict(input_tokens=input_tokens, output_tokens=len(tokens),
                 ...
                 token_limit_reached=len(tokens) >= 768,
                 time_limit_reached=type(seconds) in (int, float) and seconds >= 90)
...
def safe_completion(value):
    ...
    if any(type(value[k]) is not bool for k in ("eos_reached", "token_limit_reached", "time_limit_reached")):
        return None
    if (value["token_limit_reached"] != (value["output_tokens"] >= 768)
            or value["time_limit_reached"] != (seconds >= 90)):
        return None
    return dict(value)


def failure_receipt(exc, count, progress):
    """Bounded v4 diagnostics: no exception text, source text or model prose."""
    ...
    return dict(version=4, status="ERROR", code="SUMMARY_WORKER_FAILED", reason=reason,
...
def generate(request, count, progress):
    ...
    config = GenerationConfig(do_sample=False, num_beams=1, max_new_tokens=768,
                              max_time=90, use_cache=True,
    ...
    if len(raw.encode()) > 4096 or peak > 15_000_000_000 or elapsed > 120:
    ...
    return dict(version=4, status="OK", request_id=request["request_id"],
...
def main(argv=None, *, generator=generate):
    ...
    parser.add_argument("--one-budget-summary", action="store_true", required=True)
