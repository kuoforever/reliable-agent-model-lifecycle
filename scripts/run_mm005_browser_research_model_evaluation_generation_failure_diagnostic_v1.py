"""Plan or inspect the frozen MM-005 generation-diagnostic implementation.
...
from scripts import (  # noqa: E402
    prepare_mm005_browser_research_model_evaluation_generation_failure_diagnostic_protocol_v1 as protocol_builder,
)
...
def _verify_execution_lineage(
    ...
    if (
        current_protocol != captured_protocol
...
def _execute_authorized_diagnostic(
    ...
    authority_freeze_commit = str(authority_context["authority_freeze_commit"])
    implementation_freeze_commit = str(
        authority_context["implementation_freeze_commit"]
    )
    implementation_context = _mapping(
    ...
    expected_environment = _mapping(
        _mapping(authority.get("resource_preflight"), "$.resource_preflight").get(
            "expected_environment"
        ),
        "$.resource_preflight.expected_environment",
    )
    ...
    _require_unclaimed_execution_state(authority_context)
    lifecycle_path = ROOT / protocol.LIFECYCLE_LEASE_PATH
    output_parent_guard = recovery_io.DirectoryTreeGuard(
        ROOT, (ROOT / protocol.RUN_OUTPUT_ROOT).parent
    )
    output_parent_guard.verify()
    ...
    with (
        ...
        _require_runtime_outputs_absent(allow_lifecycle=True)
        attempt_id = secrets.token_hex(32)
        owner = result_contract.build_attempt_owner(
            implementation_freeze_commit=implementation_freeze_commit,
            preregistration_payload=preregistration_payload,
            authority_freeze_commit=authority_freeze_commit,
            execution_authority_payload=authority_payload,
...
def _published_protocol_context(*, allow_runtime_state: bool = False) -> dict[str, Any]:
    """Bind the exact PR #76 protocol/config/thirteen-source publication."""
...
def _output_topology() -> dict[str, bool]:
    return {
        ...
        "reserved_sibling_staging": bool(_reserved_sibling_staging_names()),
    }


def _validate_output_topology(topology: Mapping[str, bool]) -> None:
    if set(topology) != set(RUNTIME_OUTPUT_KEYS) | {"execution_authority"} or any(
        type(value) is not bool for value in topology.values()
    ):
...
def _claimed_execution_snapshot(
    ...
    if last_event in protocol.TERMINAL_EVENTS:
        if tail_receipt is not None:
...
def _require_reserved_sibling_staging_absent() -> None:
    ...
    reserved = _reserved_sibling_staging_names()
    if reserved:
        raise RuntimeError("reserved diagnostic sibling staging requires review")


def _claim_output(
    *, attempt_id: str, owner_payload: bytes, genesis_payload: bytes
) -> None:
...
def _run_frozen_diagnostic_session(
    *,
    ...
    if set(protocol.DIAGNOSTIC_CASE_ORDER) - set(by_id):
    ...
    with torch.inference_mode():
        for diagnostic_index, record_id in enumerate(protocol.DIAGNOSTIC_CASE_ORDER):
...
def _implementation_source_context(commit: str) -> dict[str, Any]:
    ...
    diff = set(_git_name_only_paths(result_contract.PROTOCOL_MERGE_COMMIT, commit))
...
def _git_blob_bytes(commit: str, relative: str) -> bytes:
    _validate_repository_relative_path(relative)
    process = _git_process("cat-file", "blob", f"{commit}:{relative}")
...
def _require_no_hidden_index_flags() -> None:
    ...
    process = _git_process("ls-files", "-v", "-z")
    if process.returncode != 0 or process.stderr:
