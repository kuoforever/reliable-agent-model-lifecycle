"""Build or check the frozen MM-005 environment-adaptation protocol."""
...
SOURCE_PATHS = {
    ...
    "mm004_generation_evidence": (
        "baseline/mm004-multimodal-hard-negative-data-generation-v1.json",
        "sequencing_evidence",
    ),
    "runtime_freeze": (
...
def all_source_paths() -> dict[str, tuple[str, str]]:
    """Return the closed tracked source set, including every prior image."""
...
def exclusion_registry() -> dict[str, list[str]]:
    """Recompute shared identities from upstream content, independent of old IDs."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        if _read_regular_file_once(OUTPUT_PATH) != payload:
            raise SystemExit("MM-005 frozen protocol is stale")
        print(f"MM-005 protocol verified: {contract.sha256_bytes(payload)}")
        return 0
    OUTPUT_PATH.write_bytes(payload)
    print(f"MM-005 protocol frozen: {contract.sha256_bytes(payload)}")
    return 0
...
def _add_upstream_record(
    ...
    *,
    ...
    result["target_content_sha256"].add(
        contract.content_identity("target", target)
    )
