"""Build or check the frozen MM-005 Browser Research Adapter/Verifier protocol."""
...
from fullcycle_bridge import (  # noqa: E402
    mm005_browser_research_generation as generation,
)
from scripts import run_mm005_browser_research_generation as generation_runner  # noqa: E402
...
SOURCE_PATHS = {
    ...
    "generation_contract": "src/fullcycle_bridge/mm005_browser_research_generation.py",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        if _read_regular_file_once(output_path) != payload:
            raise SystemExit("MM-005 Browser Research Adapter/Verifier protocol is stale")
        print(
            "MM-005 Browser Research Adapter/Verifier protocol verified: "
    ...
    print(
        "MM-005 Browser Research Adapter/Verifier protocol frozen: "
        f"{contract.sha256_bytes(payload)}"
    )
    return 0
...
def build_protocol() -> dict[str, Any]:
    return contract.expected_protocol(freeze_status="frozen", **protocol_inputs())
...
def protocol_inputs() -> dict[str, Any]:
    ...
    exclusions = cast(
        Mapping[str, Sequence[str]], parent_protocol["exclusion_registry"]
    )
    return {
