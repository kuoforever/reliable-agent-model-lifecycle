"""Build or check MM-005 Adapter/Verifier implementation evidence."""
...
from scripts import (  # noqa: E402
    prepare_mm005_document_chart_pdf_adapter_verifier_protocol as protocol_prepare,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        if _read_regular_file_once(output_path) != payload:
            raise SystemExit("MM-005 Adapter/Verifier implementation evidence is stale")
        print(
            "MM-005 Adapter/Verifier implementation evidence verified: "
    ...
    print(
        "MM-005 Adapter/Verifier implementation evidence written: "
        f"{evidence.sha256_bytes(payload)}"
    )
    return 0
...
def _validate_protocol_merge() -> None:
    ...
    if ancestor.returncode != 0:
        raise RuntimeError("Adapter/Verifier protocol merge is not an ancestor")
    ...
    if frozen != current:
        raise RuntimeError("Adapter/Verifier protocol changed after merge")
