"""Execute the frozen MM-005 Document/Chart/PDF model evaluation once."""
...
from fullcycle_bridge import (  # noqa: E402
    mm005_document_chart_pdf_data as data_contract,
)
from fullcycle_bridge import (  # noqa: E402
    mm005_document_chart_pdf_adapter_verifier_implementation as implementation_contract,
)
from fullcycle_bridge import (  # noqa: E402
...
def execute_frozen_protocol(*, protocol_freeze_commit: str) -> dict[str, Any]:
    ...
    expected_payloads = _bytes_mapping(inputs["image_payloads"], "$.image_payloads")
    ...
    *,
    ...
    with torch.inference_mode():
        for record in ordered:
            ...
            image = image_class.open(io.BytesIO(adapted.image_bytes)).convert("RGB")
            try:
                messages = contract.build_runtime_messages(
                    adapted.model_payload(), image
                )
                torch.cuda.synchronize()
                started = time.perf_counter()
