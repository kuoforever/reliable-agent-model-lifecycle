"""Prepare or execute the frozen MM-003 QLoRA post-training protocol."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "work/training-runs/mm003-qlora-sft-v1",
    )
    return parser
...
def execute_frozen_protocol(
    *,
    ...
    expected_output_dir = (ROOT / "work/training-runs/mm003-qlora-sft-v1").resolve()
...
def build_evidence(
    *,
    ...
    return {
        ...
        "claims": claims,
        "next_gate": "MM-003-small-vlm-post-training-result-review-v1",
        "runtime_eligible": False,
    }
...
def _independent_load_and_eval(
    *,
    ...
    predictions = {
        ...
        "producer": {
            "kind": "model",
            "model_id": f"{contract.MODEL_ID}+mm003-qlora-sft-v1",
...
def _canonicalize_adapter_artifacts(adapter_dir: Path) -> None:
    ...
    readme = (
        "# MM-003 QLoRA SFT v1 Adapter\n\n"
        "This local research Adapter is bound to the frozen MM-003 post-training "
        "protocol. It is not approved for serving, promotion, commercial use, or "
        "Runtime execution.\n"
    ).encode("utf-8")
