"""One v4 doubled-budget control on the pinned reference; no desktop or network API."""
...
from scripts.probe_public_summary_budget import (  # noqa: E402
    REASONS, STAGES, SYSTEM, parse_request, render_brief, safe_completion, sha, strict_object,
)

WORKER_SHA = "6ddd3bf1bf92d30b8dae2e0dc0a24cdb56c8c6828f50f090cfffbdc633c000fd"
SOURCE_SHA = "c3cf519cadef0e1f55c6457c7151d67553b8c87f70d79a128373dd78d9d70a1a"
REQUEST_ID = "public-source-summary-budget-20260908"


def validate_response(response, request, exit_code):
    if type(response) is not dict or type(response.get("version")) is not int or response["version"] != 4:
        raise ValueError("RESPONSE_VERSION")
    if response.get("status") == "ERROR":
        if (exit_code != 1 or set(response) != {"version", "status", "code", "reason", "stage", "model_requests", "completion"}
        ...
        if completion is not None and safe_completion(completion) is None:
            raise ValueError("COMPLETION_METADATA")
        if response["stage"] in {"RESOURCE_CHECK", "EOS_CHECK", "POST_USE_PINS", "COMPLETE"}:
            if completion is None or response["model_requests"] != 1:
                raise ValueError("MISSING_COMPLETION")
        elif completion is not None:
            raise ValueError("PREMATURE_COMPLETION")
        if response["reason"] == "GENERATION_INCOMPLETE" and (
            response["stage"] != "EOS_CHECK" or completion is None or completion["eos_reached"]
        ):
            raise ValueError("EOS_CONTRADICTION")
        return
    fixed = dict(version=4, status="OK", request_id=request["request_id"],
    ...
    for key, cap in [("input_tokens", 4096), ("output_tokens", 768), ("peak_allocated_bytes", 15_000_000_000)]:
...
def run(reference, output, *, invoke=subprocess.run):
    ...
    worker = ROOT / "scripts/probe_public_summary_budget.py"
    ...
    pins = dict(version=1, request_id=REQUEST_ID, source_sha256=SOURCE_SHA,
                ...
                generation_budget=dict(max_new_tokens=768, max_time_seconds=90,
    ...
    try:
        completed = invoke([str(ROOT / "work/gui-owl-lora-env/Scripts/python.exe"), "-I", "-B",
                            str(worker), "--one-budget-summary"], input=raw_request,
...
def main():
    ...
    parser.add_argument("--one-budget-control", action="store_true", required=True)
    parser.parse_args()
    receipt = run(ROOT / "work/public-source-summary-20260907/request.json",
                  ROOT / "work/public-source-summary-budget-20260908")
