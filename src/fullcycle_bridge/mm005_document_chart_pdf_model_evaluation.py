"""Outcome-neutral MM-005 Document/Chart/PDF model-evaluation contract."""
...
from . import mm005_document_chart_pdf_adapter_verifier as adapter_verifier
from . import mm005_document_chart_pdf_adapter_verifier_implementation as implementation
...
PROTOCOL_GATE_ID = "MM-005-document-chart-pdf-model-evaluation-protocol-v1"
EXECUTION_GATE_ID = "MM-005-document-chart-pdf-model-evaluation-execution-v1"
RESULT_REVIEW_GATE_ID = "MM-005-document-chart-pdf-model-evaluation-result-review-v1"
FAILURE_CLASSIFICATION_GATE_ID = (
    "MM-005-document-chart-pdf-model-evaluation-failure-classification-v1"
)
EXPERIMENT_ID = "mm005-document-chart-pdf-model-eval-v1"
RUN_ID = "mm005-document-chart-pdf-model-eval-r1"
SUITE_ID = "mm005-document-chart-pdf-v1"

PREREGISTRATION_PATH = (
    "configs/mm005_document_chart_pdf_model_evaluation_protocol_v1.json"
)
RUN_OUTPUT_ROOT = "work/evaluation-runs/mm005-document-chart-pdf-model-eval-v1"
...
IMPLEMENTATION_MERGE_COMMIT = "ff52da51aba534b051f9e247518fb2d20d1db1e2"
...
SYSTEM_PROMPT = (
    "You are a read-only visual document evidence-grounding model. Use only "
    "the supplied page image and canonical task payload. Return exactly one "
    "JSON object with the keys answer, evidence_refs, and page_number. "
    "evidence_refs must be an ordered array of unique region ref strings from "
    "the observation and page_number must be 1. Return no markdown or extra "
    "text. Your output has no execution authority."
)
IMAGE_TRANSPORT_MARKER = "exact_adapter_image_bytes"
...
PROTOCOL_SOURCE_PATHS = {
    ...
    "model_evaluation_runner": (
        "scripts/run_mm005_document_chart_pdf_model_evaluation.py"
    ),
}
...
METRIC_FLAGS = {
    ...
    "evidence_exact_accuracy": "evidence_exact",
    "page_exact_accuracy": "page_exact",
...
def build_prompt_projection(model_payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        {"role": "system", "content": SYSTEM_PROMPT},
        {
            "role": "user",
            "content": [
                {"type": "image", "image_transport": IMAGE_TRANSPORT_MARKER},
...
def build_runtime_messages(
    model_payload: Mapping[str, Any], image: object
) -> list[dict[str, Any]]:
    projection = build_prompt_projection(model_payload)
    return [
        dict(projection[0]),
        {
            "role": "user",
            "content": [
                ...
                dict(_sequence(projection[1]["content"], "$.prompt.content")[1]),
            ],
        },
    ]
...
def prompt_projection_registry(
    ...
    for record in _ordered_records(records):
        try:
            adapted = adapter_verifier.adapt_record(record, image_payloads)
        except adapter_verifier.MM005AdapterVerifierError as exc:
            raise MM005ModelEvaluationError(
                "ADAPTER_INPUT_INVALID", "$.input_suite"
            ) from exc
        model_payload = adapted.model_payload()
        prompt_payload = artifact_json_bytes(build_prompt_projection(model_payload))
        audit = adapted.audit_projection()
        image_binding = _mapping(
            audit.get("image_binding"), "$.adapter.audit.image_binding"
        )
        if (
            _contains_forbidden_key(model_payload)
            or set(model_payload) != set(MODEL_PAYLOAD_KEYS)
        ...
        registry.append(
            {
                ...
                "image_payload": _byte_receipt(adapted.image_bytes),
...
def expected_preregistration(
    *,
    ...
    projections = prompt_projection_registry(ordered_records, image_payloads)
    ...
    return {
        "mm005_document_chart_pdf_model_evaluation_protocol_version": (
            PROTOCOL_VERSION
        ),
        ...
        "prompt_contract": {
            ...
            "visual_transport": IMAGE_TRANSPORT_MARKER,
            "model_payload_exact_keys": list(MODEL_PAYLOAD_KEYS),
...
def build_attempt_owner(
    *, protocol_freeze_commit: str, preregistration_payload: bytes, attempt_id: str
    ...
    return {
        "mm005_document_chart_pdf_model_evaluation_attempt_owner_version": (
...
def build_case_result(
    *,
    ...
    prompt_payload = artifact_json_bytes(
        build_prompt_projection(adapted.model_payload())
    )
    audit = adapted.audit_projection()
    image_binding = _mapping(audit.get("image_binding"), "$.case.image_binding")
    return {
        "record_id": record["record_id"],
        "split": record["split"],
        "task_family_id": record["task_family_id"],
...
def build_evaluation_candidate(
    *,
    ...
    return {
        "mm005_document_chart_pdf_model_evaluation_candidate_version": (
            CANDIDATE_VERSION
        ),
        "gate_id": EXECUTION_GATE_ID,
        "run_id": RUN_ID,
...
def build_predictions(candidate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "mm005_document_chart_pdf_predictions_version": PREDICTIONS_VERSION,
        "suite_id": SUITE_ID,
...
def build_evidence(
    *,
    ...
    return {
        "mm005_document_chart_pdf_model_evaluation_evidence_version": (
            EVIDENCE_VERSION
        ),
        "gate_id": EXECUTION_GATE_ID,
        "run_id": RUN_ID,
...
def build_failure(
    *,
    ...
    return {
        "mm005_document_chart_pdf_model_evaluation_failure_version": (FAILURE_VERSION),
        "gate_id": EXECUTION_GATE_ID,
        "run_id": RUN_ID,
        "protocol_freeze_commit": protocol_freeze_commit,
        "stage": stage,
