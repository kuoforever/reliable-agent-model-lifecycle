"""Closed journal and terminal contracts for the MM-005 generation diagnostic v2.
...
EXECUTION_AUTHORITY_GATE_ID = protocol.AUTHORITY_GATE_ID
EXECUTION_GATE_ID = protocol.EXECUTION_GATE_ID
EXECUTION_AUTHORITY_PATH = (
    "configs/mm005_browser_research_model_evaluation_generation_failure_"
    "diagnostic_execution_authority_v2.json"
)
...
IMPLEMENTATION_SOURCE_PATHS = {
    "diagnostic_result_contract": (
        ...
        "generation_failure_diagnostic_result_v2.py"
    ),
    "diagnostic_runner": (
        "scripts/run_mm005_browser_research_model_evaluation_generation_"
        "failure_diagnostic_v2.py"
    ),
...
EXECUTION_AUTHORITY_SLICE_PATHS = frozenset(
    {
        ...
        (
            ...
            "diagnostic-execution-authority-v2.md"
        ),
        (
            "docs/MM-005-browser-research-model-evaluation-generation-failure-"
        ...
        (
            ...
            "failure_diagnostic_execution_authority_v2.py"
        ),
        "scripts/validate_offline.py",
        (
            "tests/test_mm005_browser_research_model_evaluation_generation_failure_"
            "diagnostic_execution_authority_v2.py"
        ),
    }
)

OUTCOME_PRECEDENCE = tuple(scientific_protocol.ALLOWED_OUTCOMES)
FAILURE_SCOPES = (
...
def parse_strict_json_bytes(payload: bytes, *, location: str) -> dict[str, Any]:
    ...
    except protocol.MM005GenerationFailureDiagnosticProtocolV2Error as exc:
...
def result_contract() -> dict[str, Any]:
    ...
    summary = {
        ...
        "implementation_base_binding": {
            "commit": IMPLEMENTATION_BASE_COMMIT,
            "unique_parent": ZERO_BANDWIDTH_MAINTENANCE_COMMIT,
            "zero_bandwidth_maintenance_commit": ZERO_BANDWIDTH_MAINTENANCE_COMMIT,
            "zero_bandwidth_maintenance_unique_parent": PROTOCOL_MERGE_COMMIT,
        ...
        "execution_authority_contract": {
            ...
            "authority_introduction_has_implementation_as_unique_parent": True,
        ...
        "output_parent_preparation_contract": {
            "work_root": protocol.WORK_ROOT_PATH,
            "output_parent": protocol.OUTPUT_PARENT_PATH,
            "public_execute_requires_published_authority": True,
            "clean_aligned_exact_authority_head_required_before_mkdir": True,
            "root_and_work_identity_revalidated_before_mkdir": True,
            "work_directory_guard_verified_before_mkdir": True,
            "exclusive_single_component_primitive": "os.mkdir",
            "parents_created": False,
            "exist_ok": False,
            "authority_lineage_sources_and_remaining_topology_revalidated_after_create": (
                True
            ),
            "root_work_and_parent_identity_revalidated_after_create": True,
            "created_parent_directory_guard_verified_before_lifecycle": True,
            "lifecycle_before_atomic_owner_and_genesis": True,
            "atomic_owner_and_genesis_before_first_heavy_boundary": True,
            "parent_creation_is_attempt_claim": False,
            "parent_creation_is_formal_telemetry": False,
            "pre_owner_terminal_scope_or_outcome_available": False,
            "post_create_pre_lifecycle_failure_may_leave_parent_only": True,
            "parent_only_continuation_or_terminal_reconciliation_forbidden": True,
            "post_lifecycle_pre_owner_failure_may_leave_lifecycle_only": True,
            "lifecycle_only_continuation_or_terminal_reconciliation_forbidden": True,
            "owner_staging_failure_remains_durable_and_unretryable": True,
            "same_privilege_toctou_eliminated": False,
        },
        "implementation_regression_contract": {
            "real_temporary_git_and_filesystem": True,
            "initial_work_root_exists": True,
            "initial_output_parent_absent": True,
            "parent_helper_mocked": False,
            "directory_tree_guard_mocked": False,
            "lease_claim_or_io_mocked": False,
            "only_executed_injected_boundary": "first_heavy_dependency_boundary",
            "fail_on_call_spies_required": [
                "socket",
                "network",
                "model_load",
                "cuda_workload",
            ],
            "owner_and_genesis_exist_before_controlled_exception": True,
            "controlled_failure_scope": "pre_record_lifecycle",
            "model_import_or_load_entered": False,
            "cuda_or_network_entered": False,
            "formal_invocation_budget_consumed": False,
        },
        "journal_contract": {
            "format": "canonical_jsonl_append_only_sha256_chain",
            "session_lifecycle_events": list(
                scientific_protocol.SESSION_LIFECYCLE_EVENTS
            ),
            "case_order": list(scientific_protocol.DIAGNOSTIC_CASE_ORDER),
            "durable_checkpoint_events": list(
                scientific_protocol.DIAGNOSTIC_CHECKPOINTS
            ),
            "per_record_checkpoint_count": len(
                scientific_protocol.DIAGNOSTIC_CHECKPOINTS
            ),
            "maximum_checkpoint_count": (
                scientific_protocol.FULL_SUCCESS_DURABLE_SUBSTAGE_EVENT_COUNT
            ),
            "event_identity_fields": list(scientific_protocol.EVENT_IDENTITY_FIELDS),
        ...
        "result_schema": {
            ...
            "record_result_count": len(scientific_protocol.DIAGNOSTIC_CASE_ORDER),
        ...
        "outcome_selection": {
            "allowed_outcomes": list(scientific_protocol.ALLOWED_OUTCOMES),
...
def build_case_result_summary(
    ...
    if type(diagnostic_index) is not int or not (
        0 <= diagnostic_index < len(scientific_protocol.DIAGNOSTIC_CASE_ORDER)
    ):
        _fail("CASE_SUMMARY_INDEX_INVALID", "$.case_result")
    case = _mapping(case_result, "$.case_result")
    record_id = scientific_protocol.DIAGNOSTIC_CASE_ORDER[diagnostic_index]
...
def failure_scope_for_journal(
    ...
    *,
    ...
    if events[-1]["event"] in scientific_protocol.TERMINAL_EVENTS:
...
def _expected_progress_event(
    *,
    ...
    if (
        previous_events
        and previous_events[-1].get("event") in scientific_protocol.TERMINAL_EVENTS
    ):
    ...
    *,
    ...
    if event == "failure_terminal_ready":
        ...
        if scope == "inter_record_transition":
            ...
            active_id = scientific_protocol.DIAGNOSTIC_CASE_ORDER[active_index]
        elif scope == "post_record_terminalization":
            final_plan = scientific_protocol.PER_RECORD_CHECKPOINT_PLANS[-1][
                "durable_events"
            ]
    ...
    if event == "success_terminal_ready":
        if (
            ...
            or completed != list(scientific_protocol.DIAGNOSTIC_CASE_ORDER)
    ...
    if session_index < len(scientific_protocol.SESSION_LIFECYCLE_EVENTS):
        expected_event = scientific_protocol.SESSION_LIFECYCLE_EVENTS[session_index]
        if (
    ...
    if observed_environment is not None:
        _fail("ENVIRONMENT_OBSERVATION_EVENT_INVALID", "$.progress")
    if len(session_events) != len(scientific_protocol.SESSION_LIFECYCLE_EVENTS):
        _fail("SESSION_LIFECYCLE_INCOMPLETE", "$.progress")
    expected_index = len(completed)
    if active_id is None:
        if expected_index >= len(scientific_protocol.DIAGNOSTIC_CASE_ORDER):
            _fail("CHECKPOINT_AFTER_COMPLETE_PLAN", "$.progress")
        expected_record = scientific_protocol.DIAGNOSTIC_CASE_ORDER[expected_index]
        expected_checkpoint = scientific_protocol.DIAGNOSTIC_CHECKPOINTS[0]
        if (
        ...
        if len(active_events) >= len(scientific_protocol.DIAGNOSTIC_CHECKPOINTS):
            _fail("ACTIVE_RECORD_PLAN_ALREADY_COMPLETE", "$.progress")
        expected_checkpoint = scientific_protocol.DIAGNOSTIC_CHECKPOINTS[
            len(active_events)
        ]
        if (
            record_id != active_id
...
def _derive_failure_scope(previous: Mapping[str, Any]) -> str:
    ...
    if completed == list(scientific_protocol.DIAGNOSTIC_CASE_ORDER):
        if count != scientific_protocol.FULL_SUCCESS_DURABLE_SUBSTAGE_EVENT_COUNT:
            _fail("POST_RECORD_FAILURE_COUNT_INVALID", "$.progress")
        return "post_record_terminalization"
    if completed != list(scientific_protocol.DIAGNOSTIC_CASE_ORDER[: len(completed)]):
    ...
    *,
    ...
    return {
        ...
        "protocol_lineage": {
            ...
            "all_protocol_sources_match_merge_blobs": True,
        },
...
def _validated_cases_from_events(
    ...
    if len(cases) != len(scientific_protocol.DIAGNOSTIC_CASE_ORDER):
        _fail("DIAGNOSTIC_CASE_COUNT_INVALID", "$.record_results")
    rebuilt: list[dict[str, Any]] = []
    for index, (record_id, raw_summary) in enumerate(
        zip(scientific_protocol.DIAGNOSTIC_CASE_ORDER, cases, strict=True)
    ):
...
def _validated_environment(value: object) -> dict[str, Any]:
    ...
    required = set(scientific_protocol.OBSERVED_ENVIRONMENT_FIELDS)
