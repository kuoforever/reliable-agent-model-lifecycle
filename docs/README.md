...
| Browser Research generation-failure diagnostic result review v2 | [MM-005 Browser Research generation-failure diagnostic result review v2](MM-005-browser-research-model-evaluation-generation-failure-diagnostic-result-review-v2.md) | Active closeout: the one authorized invocation is consumed and closes as an authenticated `pre_record_lifecycle` / `diagnostic_inconclusive` failure; no diagnostic record, model evaluation, measurement, root cause, remediation, recovery, retry, or Runtime claim exists, while the safe tracked review binds four preserved ignored receipts including the lifecycle lease and needs zero LFS payload bytes |
