...
| Browser Research generation-failure diagnostic execution authority v1 | [MM-005 Browser Research generation-failure diagnostic execution authority](MM-005-browser-research-model-evaluation-generation-failure-diagnostic-execution-authority-v1.md) | Current authority-only gate: a canonical 2,706-byte artifact binds PR #77, four dependency receipts, exact Windows/CUDA environment, resource caps, one invocation, and zero retry; it consumes no attempt and its sole successor after clean merge is the one-shot diagnostic execution |
