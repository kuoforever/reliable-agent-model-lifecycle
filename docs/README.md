...
| Browser Research model-evaluation failure classification v1 | [MM-005 Browser Research failure classification](MM-005-browser-research-model-evaluation-failure-classification-v1.md) | v1 consumed owner-only after external controller interruption; exact progress and metrics unavailable, no retry/result/repeatability claim, and a separate v2 recovery protocol is locked |
