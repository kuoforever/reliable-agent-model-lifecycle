...
| Repository CI/LFS zero-bandwidth maintenance v2 | [Maintenance contract](repository-ci-lfs-zero-bandwidth-v2.md) | Closed and published through PR #82 as `e5e618b491a3dc38dbed9cdcd4c6c384f2df0f54`: immutable hydrated-anchor inheritance for automatic CI, separate acknowledged manual hydration, and zero automatic LFS payload bytes |
| Repository CI exact-head checkout v1 | [Transport correction contract](repository-ci-lfs-zero-bandwidth-v2.md#exact-event-head-checkout-prerequisite) | Published through PR #83 as `8c679eba08a979fb60bfd87fbe8c73c8725d89c0`: both automatic jobs check out the exact GitHub event head, disable credential persistence, and fail unless strict 40-hex expected SHA equals `git rev-parse HEAD` |
...
| Browser Research generation-failure diagnostic protocol v1 | [MM-005 Browser Research generation-failure diagnostic protocol](MM-005-browser-research-model-evaluation-generation-failure-diagnostic-protocol-v1.md) | Published through PR #76 as `9c90c5e...99c04`: the 57,143-byte preregistration (`13d18081...992d6`) binds PR #75, seven explicit 18-event record plans, a 126-event full-success grammar, and the post-record terminalization-failure window; no diagnostic execution occurred |
...
| Browser Research generation-failure diagnostic protocol v2 | [MM-005 Browser Research generation-failure diagnostic protocol v2](MM-005-browser-research-model-evaluation-generation-failure-diagnostic-protocol-v2.md) | Published through PR #81 as `eb2aea3...b3fc9`; freezes the new identity, exclusive output-parent preparation, and mandatory real-filesystem implementation regression without authority |
| Browser Research generation-failure diagnostic implementation v2 | [MM-005 Browser Research generation-failure diagnostic implementation v2](MM-005-browser-research-model-evaluation-generation-failure-diagnostic-implementation-v2.md) | PR #84 published initial I1 (`e185c76...981b8`); PR #85 published final stage-compatible I2 (`ac052a3...ef59`) as I1's unique child, preserving both exact-11-path M2 deltas, the non-empty in-slice correction, I1 source introductions, and final I2 bytes; it published no authority or execution state |
| Browser Research generation-failure diagnostic execution authority v2 | [MM-005 Browser Research generation-failure diagnostic execution authority v2](MM-005-browser-research-model-evaluation-generation-failure-diagnostic-execution-authority-v2.md) | Closed historical authority grant: the canonical 2,944-byte artifact (`b638a7a7...39f0`) binds I2, four critical dependency receipts, the frozen expected 17-field environment, resource caps, one invocation, and zero retry; its one invocation is now spent and its effective state is governed by the authenticated result review below |
| Browser Research generation-failure diagnostic result review v2 | [MM-005 Browser Research generation-failure diagnostic result review v2](MM-005-browser-research-model-evaluation-generation-failure-diagnostic-result-review-v2.md) | Completed through PR #87 (`5044f5e...f39a2`): the one authorized invocation is consumed and closes as an authenticated `pre_record_lifecycle` / `diagnostic_inconclusive` failure; no diagnostic record, model evaluation, measurement, root cause, remediation, recovery, retry, Runtime claim, or automatic successor exists, while the safe tracked review binds four preserved ignored receipts including the lifecycle lease and needs zero LFS payload bytes |
