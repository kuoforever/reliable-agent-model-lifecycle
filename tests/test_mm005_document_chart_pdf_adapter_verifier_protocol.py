...
PROTOCOL_BYTES = 126_032
PROTOCOL_SHA256 = (
    "sha256:4715134d7bd1f8ae54275764f342bf5a8974cc491298dbefd52971aab876c64a"
)


class MM005DocumentChartPdfAdapterVerifierProtocolTests(unittest.TestCase):
    @classmethod
    ...
    def test_upstream_receipts_and_narrow_claims_are_closed(self) -> None:
        ...
        self.assertEqual(
            upstream["generation_claims"],
            contract.generation.EXECUTION_CLAIMS,
        )
    ...
    def test_adapter_projection_is_closed_and_model_payload_isolated(self) -> None:
        registry = {
            item["record_id"]: item
        ...
        for record in self.records:
            projection = contract.project_record(record, self.image_receipts)
            self.assertEqual(
                set(projection),
                {
            ...
            self.assertNotIn(projection["image_binding"]["path"].encode(), model_bytes)
            ...
            self.assertEqual(
                frozen["image_sha256"], projection["image_binding"]["sha256"]
            )

    def test_image_bytes_or_record_binding_tamper_fails_closed(self) -> None:
        tampered_outputs = dict(self.inputs["output_payloads"])
        image_path = next(path for path in tampered_outputs if path.endswith(".png"))
        image_payload = tampered_outputs[image_path]
        tampered_outputs[image_path] = image_payload[:-1] + bytes(
            [image_payload[-1] ^ 1]
        )
        with self.assertRaisesRegex(
            contract.MM005AdapterVerifierProtocolError,
            "IMAGE_RECEIPT_MISMATCH",
        ):
            contract.dataset_context(tampered_outputs)
        ...
        with self.assertRaisesRegex(
            ...
            contract.project_record(record, missing_binding)

    def test_output_compiler_accepts_only_the_frozen_json_contract(self) -> None:
        ...
        self.assertTrue(
            parent.verify_candidate(valid, self.records[0])["joint_correct"]
        )
        ...
        invalid_outputs = (
            ...
            '{"answer":"x","evidence_refs":["page-01"],"page_number":2}',
            json.dumps(
                {
                    "answer": "x" * 8_193,
    ...
    def test_answer_normalization_is_nfc_ascii_space_trim_exact(self) -> None:
        ...
        self.assertTrue(
            parent.verify_candidate(compiled_space, record)["joint_correct"]
        )
    ...
    def test_verifier_registry_freezes_all_positive_and_negative_controls(
        self,
    ) -> None:
        cases = self.protocol["verifier_case_registry"]
        self.assertEqual(len(cases), 160)
        self.assertEqual(len({case["case_id"] for case in cases}), 160)
        self.assertEqual(
        ...
        for case in cases:
            ...
            elif case["case_kind"] in {"wrong_answer", "wrong_evidence"}:
                self.assertTrue(case["compiler_valid"])
                self.assertFalse(case["verdict"]["joint_correct"])
            else:
                self.assertFalse(case["compiler_valid"])
                self.assertFalse(case["verdict"]["joint_correct"])
    ...
    def test_authority_and_implementation_remain_deferred(self) -> None:
        ...
        self.assertEqual(len(contract.REQUIRED_GATES), 16)

    def test_protocol_scope_has_no_model_network_or_runtime_imports(self) -> None:
        forbidden = {
            "http",
            "openai",
            "peft",
        ...
        for relative in prepare.SOURCE_PATHS.values():
            if not relative.startswith(
                (
                    "src/fullcycle_bridge/mm005_document_chart_pdf_adapter_verifier",
                    "scripts/prepare_mm005_document_chart_pdf_adapter_verifier",
                )
            ):
                continue
