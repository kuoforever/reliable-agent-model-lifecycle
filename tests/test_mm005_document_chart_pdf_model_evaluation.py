...
from fullcycle_bridge import (  # noqa: E402
    mm005_document_chart_pdf_adapter_verifier as adapter_verifier,
)
from fullcycle_bridge import (  # noqa: E402
    mm005_document_chart_pdf_model_evaluation as contract,
)
from scripts import (  # noqa: E402
    prepare_mm005_document_chart_pdf_model_evaluation as prepare,
)
from scripts import (  # noqa: E402
    run_mm005_document_chart_pdf_model_evaluation as runner,
)
...
class MM005DocumentChartPdfModelEvaluationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.images = {
            ...
            for path, payload in cls.inputs["image_payloads"].items()
        }
    ...
    def test_frozen_protocol_reconstructs_exact_tracked_bytes(self) -> None:
        ...
        self.assertEqual(len(tracked), 58_414)
        self.assertEqual(
    ...
    def test_prompt_projection_is_closed_gold_free_and_byte_bound(self) -> None:
        ...
        rebuilt = contract.prompt_projection_registry(self.records, self.images)
        ...
        self.assertEqual(
            sum(item["model_payload"]["bytes"] for item in registry), 31_430
        )
        self.assertEqual(
            sum(item["image_payload"]["bytes"] for item in registry), 314_128
        )
        forbidden_keys = {
            "expected_output",
            "family_id",
        ...
        for record in self.ordered_records:
            adapted = adapter_verifier.adapt_record(record, self.images)
            model_payload = adapted.model_payload()
            prompt = contract.build_prompt_projection(model_payload)
            self.assertFalse(self._json_keys(model_payload) & forbidden_keys)
            self.assertNotIn(
                str(adapted.audit_projection()["image_binding"]["path"]),
    ...
    def test_perfect_metrics_are_total_across_all_registered_groups(self) -> None:
        ...
        for name in contract.METRIC_FLAGS:
        ...
        for grouping in (
            ...
            for group in metrics[grouping].values():
                self.assertTrue(
                    all(group[name]["value"] == 1.0 for name in contract.METRIC_FLAGS)
                )
    ...
    def test_wrong_and_invalid_outputs_remain_outcome_neutral(self) -> None:
        ...
        self.assertEqual(wrong["evidence_exact_accuracy"]["value"], 0.0)
        self.assertEqual(wrong["page_exact_accuracy"]["value"], 1.0)
    ...
    def test_fake_model_lifecycle_runs_exactly_thirty_two_ordered_calls(
        ...
        self.assertNotIn("image_path", prompt_text)
