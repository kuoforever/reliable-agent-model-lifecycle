...
class MM005BrowserResearchGenerationFailureDiagnosticResultV2Tests(unittest.TestCase):
    ...
    def _session(self, *, length: int = 6) -> bytes:
        journal = b""
        for event in scientific_protocol.SESSION_LIFECYCLE_EVENTS[:length]:
    ...
    def _case_summary(self, index: int) -> dict[str, Any]:
        ...
        return {
            "record_id": scientific_protocol.DIAGNOSTIC_CASE_ORDER[index],
    ...
    def _append_record_prefix(self, journal: bytes, index: int, length: int) -> bytes:
        record_id = scientific_protocol.DIAGNOSTIC_CASE_ORDER[index]
        for event in scientific_protocol.DIAGNOSTIC_CHECKPOINTS[:length]:
            journal = self._append(
                journal,
                event,
                record_id=record_id,
    ...
    def _git_at(root: Path, *args: str) -> str:
        ...
        if process.returncode != 0:
            self_contained = " ".join(args)
            raise AssertionError(
                f"Git fixture command failed: {self_contained}: {process.stderr}"
            )
        return process.stdout.strip()

    def _real_execution_context(
        self,
        temporary_root: Path,
        *,
        insert_implementation_intermediate: bool = False,
        repository_name: str = "repository",
    ) -> tuple[Path, runner._ValidatedExecutionContext]:
        """Create a clean, aligned temporary lineage with one synthetic authority.

        The synthetic authority is test-only input to the private closed core. It
        neither exercises nor exposes the production published-authority entry.
        """

        repository = temporary_root / repository_name
        ...
        if clone.returncode != 0:
            self.fail(f"temporary repository clone failed: {clone.stderr}")
        self._git_at(repository, "config", "user.name", "MM005 Test")
        self._git_at(repository, "config", "user.email", "mm005@example.invalid")
        protocol_commit = contract.PROTOCOL_MERGE_COMMIT
        zero_bandwidth_maintenance_commit = contract.ZERO_BANDWIDTH_MAINTENANCE_COMMIT
        self._git_at(
            repository,
            "checkout",
            "-B",
        ...
        self.assertEqual(
            ...
            [zero_bandwidth_maintenance_commit, protocol_commit],
        )
        self.assertEqual(
            self._git_at(
                repository,
        ...
        if insert_implementation_intermediate:
            self._git_at(
                ...
                "--allow-empty",
                "-m",
                "synthetic unreviewed implementation intermediate",
            )
            intermediate_commit = self._git_at(repository, "rev-parse", "HEAD")

        implementation_source_paths = set(contract.IMPLEMENTATION_SOURCE_PATHS.values())
        for relative in sorted(runner.IMPLEMENTATION_SLICE_PATHS):
            destination = repository / relative
            destination.parent.mkdir(parents=True, exist_ok=True)
            source = ROOT / relative
            payload = source.read_bytes() if source.exists() else b""
            if relative not in implementation_source_paths:
                payload += b"\nimplementation-v2-real-fs-fixture\n"
        ...
        self._git_at(
            ...
            "commit",
            "-m",
            "freeze implementation v2 fixture",
        )
        implementation_commit = self._git_at(repository, "rev-parse", "HEAD")
        ...
        self.assertEqual(
            ...
            [implementation_commit, expected_implementation_parent],
        )
        self.assertEqual(
            set(
                runner._git_name_only_paths_at(
                    repository, implementation_base_commit, implementation_commit
                )
            ),
        ...
        (repository / protocol.WORK_ROOT_PATH).mkdir(exist_ok=True)
        ...
        source_paths.update(
            {
                ...
                for name, relative in sorted(
                    contract.IMPLEMENTATION_SOURCE_PATHS.items()
                )
            }
        )
        source_paths.update(
        ...
        tracked_sources = tuple(
            runner._TrackedExecutionSource(
                name=name,
        ...
        context = runner._validated_execution_context(
            ...
            preregistration_payload=(
                repository / protocol.PREREGISTRATION_PATH
            ).read_bytes(),
            implementation_context_payload=contract.artifact_json_bytes(
                implementation_context
            ),
    ...
    def test_protocol_merge_config_sources_and_three_file_closure(
        self,
    ) -> None:
        self.assertEqual(
            contract.PROTOCOL_MERGE_COMMIT, "eb2aea3ca1eb5d82e823f7fc7a6aac7b5beb3fc9"
        )
        self.assertEqual(
            contract.ZERO_BANDWIDTH_MAINTENANCE_COMMIT,
        ...
        self.assertEqual(len(protocol.PROTOCOL_SOURCE_PATHS), 2)
        ...
        self.assertEqual(context["protocol_source_files"], 2)
        self.assertEqual(
            context["zero_bandwidth_maintenance_commit"],
            contract.ZERO_BANDWIDTH_MAINTENANCE_COMMIT,
        )
        self.assertEqual(
            context["implementation_base_commit"], contract.IMPLEMENTATION_BASE_COMMIT
        )
    ...
    def test_contract_closes_schema_authority_claims_and_routes(self) -> None:
        ...
        base_binding = value["implementation_base_binding"]
        authority_contract = value["execution_authority_contract"]
        owner_contract = value["owner_contract"]
        parent_contract = value["output_parent_preparation_contract"]
        regression_contract = value["implementation_regression_contract"]
        ...
        self.assertEqual(base_binding["commit"], contract.IMPLEMENTATION_BASE_COMMIT)
        self.assertEqual(
            base_binding["unique_parent"],
            contract.ZERO_BANDWIDTH_MAINTENANCE_COMMIT,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            regression_contract["fail_on_call_spies_required"],
            ["socket", "network", "model_load", "cuda_workload"],
        )
        authority_payload = json.loads(self.authority_payload)
        self.assertEqual(
            authority_payload["zero_bandwidth_maintenance_commit"],
            contract.ZERO_BANDWIDTH_MAINTENANCE_COMMIT,
        )
        self.assertEqual(
        ...
        changed_authority = copy.deepcopy(authority_payload)
        changed_authority["zero_bandwidth_maintenance_commit"] = "d" * 40
        with self.assertRaises(contract.MM005GenerationFailureDiagnosticResultError):
            contract.build_attempt_owner(
                implementation_freeze_commit=self.implementation_commit,
                preregistration_payload=self.preregistration_payload,
        ...
        self.assertEqual(
            parent_contract["exclusive_single_component_primitive"], "os.mkdir"
        )
        self.assertFalse(parent_contract["parents_created"])
        self.assertFalse(parent_contract["exist_ok"])
        self.assertTrue(
            parent_contract[
        ...
        self.assertTrue(
            parent_contract["atomic_owner_and_genesis_before_first_heavy_boundary"]
        )
        self.assertFalse(parent_contract["parent_creation_is_attempt_claim"])
        self.assertTrue(
            parent_contract[
        ...
        self.assertFalse(parent_contract["same_privilege_toctou_eliminated"])
        self.assertTrue(regression_contract["real_temporary_git_and_filesystem"])
        self.assertEqual(
            regression_contract["only_executed_injected_boundary"],
            "first_heavy_dependency_boundary",
        )
        self.assertFalse(regression_contract["formal_invocation_budget_consumed"])
    ...
    def test_attempt_owner_is_canonical_private_and_strict(self) -> None:
        ...
        self.assertEqual(
            self.owner["zero_bandwidth_maintenance_commit"],
            contract.ZERO_BANDWIDTH_MAINTENANCE_COMMIT,
        )
        self.assertEqual(
        ...
        changed = copy.deepcopy(self.owner)
        changed["zero_bandwidth_maintenance_commit"] = "d" * 40
        with self.assertRaises(contract.MM005GenerationFailureDiagnosticResultError):
            contract.validate_attempt_owner(
                changed,
                implementation_freeze_commit=self.implementation_commit,
        ...
        for value in ("B" * 64, "b" * 63, "../secret", True):
            # These guards cover only the registered Python socket APIs, the
            # model-load-capable production boundary, and the CUDA workload;
            # they do not claim interception of native/subprocess networking.
            with (
    ...
    def test_genesis_session_sequence_hash_and_environment_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            list(scientific_protocol.SESSION_LIFECYCLE_EVENTS),
        )
    ...
    def test_strict_json_duplicate_noncanonical_partial_and_reseal_fail(self) -> None:
        ...
        value = json.loads(journal)
        value["implementation_base_commit"] = "d" * 40
        tampered = contract.artifact_json_bytes(value)
        with self.assertRaises(contract.MM005GenerationFailureDiagnosticResultError):
            contract.validate_progress_journal(
                tampered,
    ...
    def test_all_seven_by_eighteen_checkpoints_are_exact(self) -> None:
        ...
        self.assertEqual(
            events[-1]["completed_record_ids"],
            list(scientific_protocol.DIAGNOSTIC_CASE_ORDER),
        )
    ...
    def test_full_success_is_133_frames_and_builds_text_free_result(self) -> None:
        ...
        self.assertEqual(
            ...
            dict(
                scientific_protocol.PER_RECORD_CHECKPOINT_PLANS[-1]["durable_events"][
                    -1
                ]
            ),
        )
        ...
        self.assertTrue(
            result["protocol_lineage"]["all_protocol_sources_match_merge_blobs"]
        )
        self.assertNotIn(
            "all_thirteen_protocol_sources_match_merge_blobs",
            ...
            {**self.resources, "elapsed_seconds": 1},
            {
                **self.resources,
                "peak_gpu_allocated_bytes": (
                    contract.v2.RESOURCE_CAPS["peak_gpu_allocated_bytes"] + 1
                ),
            },
        ):
        ...
        for count in range(1, 7):
            with self.subTest(count=count):
                ...
                self.assertEqual(
                    ...
                    list(scientific_protocol.DIAGNOSTIC_CASE_ORDER[:count]),
                )
                self.assertEqual(
                    terminal["active_record_id"],
                    scientific_protocol.DIAGNOSTIC_CASE_ORDER[count],
                )
        ...
        for index in range(7):
            ...
            for length in range(1, 18):
                with self.subTest(index=index, length=length):
                    ...
                    self.assertEqual(
                        terminal["active_record_id"],
                        scientific_protocol.DIAGNOSTIC_CASE_ORDER[index],
                    )
    ...
    def test_case_summary_contains_no_arbitrary_model_text(self) -> None:
        ...
        case = {
            "record_id": scientific_protocol.DIAGNOSTIC_CASE_ORDER[0],
    ...
    def test_record_order_cross_record_and_checkpoint_mutations_fail(self) -> None:
        base = self._session()
        wrong_id = scientific_protocol.DIAGNOSTIC_CASE_ORDER[1]
        with self.assertRaises(contract.MM005GenerationFailureDiagnosticResultError):
            self._append(
                base,
                scientific_protocol.DIAGNOSTIC_CHECKPOINTS[0],
                record_id=wrong_id,
                diagnostic_index=1,
            )
        ...
        with self.assertRaises(contract.MM005GenerationFailureDiagnosticResultError):
            self._append(
                base,
                scientific_protocol.DIAGNOSTIC_CHECKPOINTS[1],
                record_id=scientific_protocol.DIAGNOSTIC_CASE_ORDER[1],
                diagnostic_index=1,
            )
        with self.assertRaises(contract.MM005GenerationFailureDiagnosticResultError):
            self._append(
                base,
                scientific_protocol.DIAGNOSTIC_CHECKPOINTS[2],
                record_id=scientific_protocol.DIAGNOSTIC_CASE_ORDER[0],
                diagnostic_index=0,
            )
    ...
    def test_result_failure_reseal_missing_and_extra_fields_fail(self) -> None:
        ...
        for mutation in ("missing", "extra", "reseal", "base_reseal"):
            changed = copy.deepcopy(result)
            if mutation == "missing":
                changed.pop("limitations")
            elif mutation == "extra":
                changed["unexpected"] = False
            elif mutation == "base_reseal":
                changed["protocol_lineage"]["implementation_base_commit"] = "d" * 40
                without = {
                    key: value
        ...
        changed_context = self._implementation_context()
        changed_context["implementation_base_commit"] = "d" * 40
        with self.assertRaises(contract.MM005GenerationFailureDiagnosticResultError):
            contract.build_diagnostic_result(
                **{**result_inputs, "implementation_context": changed_context}
            )
    ...
    def test_real_pre_parent_failures_leave_zero_new_runtime_state(self) -> None:
        entered_heavy = False

        def forbidden_heavy(*_args: object) -> dict[str, Any]:
            nonlocal entered_heavy
            entered_heavy = True
            self.fail("first-heavy boundary must remain unreachable")

        with tempfile.TemporaryDirectory() as temporary:
            repository, context = self._real_execution_context(Path(temporary))
            expected_chain = (
                (
                    context.zero_bandwidth_maintenance_commit,
            ...
            for child, expected_parent in expected_chain:
                runner._require_unique_parent_at(repository, child, expected_parent)
            self.assertEqual(
                set(
                    runner._git_name_only_paths_at(
                        repository,
            ...
            with self.assertRaisesRegex(RuntimeError, "unique direct parent"):
                runner._require_unique_parent_at(
                    repository,
                    context.implementation_freeze_commit,
                    context.protocol_merge_commit,
                )

            non_direct_repository, non_direct_context = self._real_execution_context(
                Path(temporary),
                insert_implementation_intermediate=True,
                repository_name="r2",
            )
            non_direct_parent = non_direct_repository / protocol.OUTPUT_PARENT_PATH
            with self.assertRaisesRegex(RuntimeError, "unique direct parent"):
                runner._execute_validated_context(
                    non_direct_context, first_heavy_boundary=forbidden_heavy
                )
            self.assertFalse(os.path.lexists(non_direct_parent))

            parent = repository / protocol.OUTPUT_PARENT_PATH

            dirty = repository / "unexpected-drift.txt"
            dirty.write_text("dirty\n", encoding="utf-8")
            with self.assertRaisesRegex(RuntimeError, "clean aligned"):
                runner._execute_validated_context(
                    context, first_heavy_boundary=forbidden_heavy
                )
            dirty.unlink()
            ...
            self._git_at(repository, "switch", "-c", "wrong-branch")
            with self.assertRaisesRegex(RuntimeError, "clean aligned"):
                runner._execute_validated_context(
                    context, first_heavy_boundary=forbidden_heavy
                )
            self._git_at(repository, "switch", "master")

            flagged = next(iter(sorted(contract.IMPLEMENTATION_SOURCE_PATHS.values())))
            self._git_at(repository, "update-index", "--skip-worktree", flagged)
            with self.assertRaisesRegex(RuntimeError, "index flag"):
                runner._execute_validated_context(
                    context, first_heavy_boundary=forbidden_heavy
                )
            self._git_at(repository, "update-index", "--no-skip-worktree", flagged)

            work = repository / protocol.WORK_ROOT_PATH
            work.rmdir()
            ...
            parent.mkdir()
            collision_bytes = b"collision-sentinel"
            (parent / "sentinel.bin").write_bytes(collision_bytes)
            with self.assertRaises(FileExistsError):
                runner._execute_validated_context(
                    context, first_heavy_boundary=forbidden_heavy
                )
            self.assertEqual((parent / "sentinel.bin").read_bytes(), collision_bytes)

            topology = runner._output_topology_at(repository)
            self.assertTrue(topology["output_parent"])
            self.assertFalse(
                any(
                    topology[name]
                    for name in (
            ...
            self.assertFalse(entered_heavy)

    def test_real_post_create_rechecks_preserve_phase_exact_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            repository, context = self._real_execution_context(Path(temporary))
            parent = repository / protocol.OUTPUT_PARENT_PATH
            os.mkdir(parent)

            dirty = repository / "post-create-drift.txt"
            dirty.write_text("dirty\n", encoding="utf-8")
            with self.assertRaisesRegex(RuntimeError, "clean aligned"):
                runner._revalidate_execution_context(context, phase="post_parent")
            self.assertTrue(parent.is_dir())
            dirty.unlink()
            topology = runner._output_topology_at(repository)
            self.assertTrue(topology["output_parent"])
            self.assertFalse(topology["lifecycle_lease_root"])
            self.assertFalse(topology["output_root"])

            parent.rmdir()
            parent.write_bytes(b"identity-replacement")
            with self.assertRaises(RuntimeError):
                runner._revalidate_execution_context(context, phase="post_parent")
            self.assertEqual(parent.read_bytes(), b"identity-replacement")
            parent.unlink()
            os.mkdir(parent)

            lifecycle_path = repository / protocol.LIFECYCLE_LEASE_PATH
            runner.recovery_io.ensure_lock_directory(
                lifecycle_path,
            ...
            with runner.recovery_io.ProgressLease(lifecycle_path) as lifecycle:
                lifecycle.verify()
                runner._revalidate_execution_context(context, phase="lifecycle")

            authority_context = {
                "authority_freeze_commit": context.authority_freeze_commit,
                "authority_payload": context.authority_payload,
            }
            with self.assertRaisesRegex(RuntimeError, "prohibits a new attempt"):
                runner._require_unclaimed_execution_state_at(
                    repository, authority_context
                )

            heavy_entered = False

            def forbidden_heavy(*_args: object) -> dict[str, Any]:
                nonlocal heavy_entered
                heavy_entered = True
                self.fail("lifecycle-only evidence cannot continue")

            with self.assertRaises(FileExistsError):
                runner._execute_validated_context(
                    context, first_heavy_boundary=forbidden_heavy
                )
            self.assertFalse(heavy_entered)
            self.assertFalse(
                any(
                    os.path.lexists(repository / relative)
                    for relative in (
                        protocol.ATTEMPT_OWNER_PATH,
                        protocol.PROGRESS_PATH,
                        protocol.SUCCESS_RESULT_PATH,
                        protocol.FAILURE_PATH,
                    )
                )
            )

            staging = (repository / protocol.RUN_OUTPUT_ROOT).with_name(
                f".{Path(protocol.RUN_OUTPUT_ROOT).name}.owner-{'d' * 64}"
            )
            os.mkdir(staging)
            with self.assertRaisesRegex(RuntimeError, "staging requires review"):
                runner._claim_output(
                    root=repository,
                    attempt_id="e" * 64,
                    owner_payload=self.owner_payload,
                    genesis_payload=self._session(length=1),
                )

            topology = runner._output_topology_at(repository)
            self.assertTrue(topology["output_parent"])
            self.assertTrue(topology["lifecycle_lease_root"])
            self.assertTrue(topology["lifecycle_lease"])
            self.assertTrue(topology["reserved_sibling_staging"])
            self.assertFalse(
                any(
                    topology[name]
                    for name in (
    ...
    def test_parent_only_public_route_never_calls_executor_or_reconcile(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            repository, context = self._real_execution_context(Path(temporary))
            os.mkdir(repository / protocol.OUTPUT_PARENT_PATH)
            authority_context = {
                "authority_freeze_commit": context.authority_freeze_commit,
                "authority_payload": context.authority_payload,
                "published": True,
            ...
            with (
                ...
                mock.patch.object(runner, "_execute_authorized_diagnostic") as execute,
                mock.patch.object(runner, "_reconcile_claimed_execution") as reconcile,
                self.assertRaisesRegex(RuntimeError, "parent-only evidence"),
            ):
                runner.run(mode="execute")
    ...
    def test_real_fresh_parent_claim_reaches_only_injected_first_heavy_boundary(
        self,
    ) -> None:
        """Use real Git/FS guards, leases, claim I/O, and failure persistence."""
        ...
        before_modules = {
            ...
            if name.split(".", maxsplit=1)[0] in banned_modules
        }
        calls: list[str] = []

        with tempfile.TemporaryDirectory() as temporary:
            repository, context = self._real_execution_context(Path(temporary))
            output_parent = repository / protocol.OUTPUT_PARENT_PATH
            self.assertTrue((repository / protocol.WORK_ROOT_PATH).is_dir())
            self.assertFalse(os.path.lexists(output_parent))

            def controlled_first_heavy_boundary(
                validated: runner._ValidatedExecutionContext,
                lifecycle: runner.recovery_io.ProgressLease,
                journal: runner.recovery_io.ProgressLease,
                work_guard: runner.recovery_io.DirectoryTreeGuard,
                parent_guard: runner.recovery_io.DirectoryTreeGuard,
                output_guard: runner.recovery_io.DirectoryTreeGuard,
                owner_payload: bytes,
            ) -> dict[str, Any]:
                self.assertIs(validated, context)
                for guard in (work_guard, parent_guard, output_guard):
                    guard.verify()
                lifecycle.verify()
                self.assertEqual(
                    journal.read(),
                    (repository / protocol.PROGRESS_PATH).read_bytes(),
                )
                self.assertEqual(
                    owner_payload,
                    (repository / protocol.ATTEMPT_OWNER_PATH).read_bytes(),
                )
                calls.append("after_owner_and_genesis")
                raise RuntimeError("controlled-first-heavy-boundary")

            with (
                mock.patch.object(
                    socket,
                    "socket",
                    side_effect=AssertionError("socket boundary entered"),
                ) as socket_spy,
                mock.patch.object(
                    socket,
                    "create_connection",
                    side_effect=AssertionError("socket create_connection API entered"),
                ) as network_spy,
                mock.patch.object(
                    runner,
                    "_first_heavy_dependency_boundary",
                    side_effect=AssertionError(
                        "model-load-capable production boundary entered"
                    ),
                ) as model_load_spy,
                mock.patch.object(
                    runner,
                    "_run_frozen_diagnostic_session",
                    side_effect=AssertionError("CUDA workload entered"),
                ) as cuda_workload_spy,
                self.assertRaisesRegex(RuntimeError, "controlled-first-heavy-boundary"),
            ):
                runner._execute_validated_context(
                    context,
                    first_heavy_boundary=controlled_first_heavy_boundary,
                )
            ...
            for boundary_spy in (
                ...
                boundary_spy.assert_not_called()

            self.assertEqual(calls, ["after_owner_and_genesis"])
            topology = runner._output_topology_at(repository)
            self.assertTrue(
                all(
                    topology[name]
            ...
            self.assertFalse(topology["success_result"])
            self.assertFalse(topology["reserved_sibling_staging"])
            owner_payload = (repository / protocol.ATTEMPT_OWNER_PATH).read_bytes()
            progress_payload = (repository / protocol.PROGRESS_PATH).read_bytes()
            events = contract.validate_progress_journal(
                progress_payload,
            ...
            failure = contract.parse_strict_json_bytes(
                (repository / protocol.FAILURE_PATH).read_bytes(),
                location="$.failure",
            )
            self.assertEqual(failure["failure_scope"], "pre_record_lifecycle")
            self.assertEqual(failure["exception_type"], "RuntimeError")
            self.assertTrue(failure["claims"]["diagnostic_attempt_consumed"])
            self.assertFalse(failure["claims"]["diagnostic_executed"])
            self.assertEqual(
                failure["decision"]["selected_outcome"],
    ...
    def test_fake_case_producer_emits_exact_eighteen_events_and_closes_images(
        ...
        self.assertEqual(events, list(scientific_protocol.DIAGNOSTIC_CHECKPOINTS))
    ...
    def test_runner_ast_has_explicit_modes_lazy_heavy_imports_and_no_authority_bypass(
        ...
        ordered_core = functions["_execute_validated_context"]
        self.assertIsNotNone(ordered_core)
        assert ordered_core is not None
        ordered_tokens = (
            ...
            "DirectoryTreeGuard(root, output_parent)",
        ...
        positions = [ordered_core.index(token) for token in ordered_tokens]
        self.assertEqual(positions, sorted(positions))
        self.assertNotIn("mock", ordered_core)
        mkdir_calls = [
            node
            for node in ast.walk(ast.parse(ordered_core))
        ...
        self.assertEqual(len(mkdir_calls[0].args), 1)
        self.assertEqual(mkdir_calls[0].keywords, [])
        public_execute = functions["_execute_authorized_diagnostic"]
        self.assertIsNotNone(public_execute)
        assert public_execute is not None
        self.assertIn(
            "_validated_execution_context_from_published_authority",
            public_execute,
        )
        self.assertNotIn("bypass", public_execute.casefold())
