...
from scripts import (  # noqa: E402
    reassess_tool_router_fp32_attached_offline_artifact_eligibility as builder,
)
...
class OfflineArtifactEligibilityReassessmentTests(unittest.TestCase):
    ...
    def _validate(
        ...
        *,
        ...
        return contract.validate_reassessment_evidence(
    ...
    def assert_code(self, code: str, callback: Callable[[], object]) -> None:
        with self.assertRaisesRegex(contract.EligibilityReassessmentError, code):
            callback()

    def test_outcome_neutral_rubric_accepts_only_complete_requirements(self) -> None:
        complete = {name: True for name in contract.REQUIREMENT_KEYS}
        decision = contract.classify_offline_artifact_eligibility(complete)
        self.assertTrue(decision["offline_artifact_eligible"])
        ...
        for missing in contract.REQUIREMENT_KEYS:
            with self.subTest(missing=missing):
                result = contract.classify_offline_artifact_eligibility(
                    {**complete, missing: False}
                )
                self.assertFalse(result["offline_artifact_eligible"])
                self.assertEqual(result["blocking_finding_count"], 1)
                self.assertEqual(
                    result["classification"], contract.INCOMPLETE_CLASSIFICATION
                )

    def test_draft_preregistration_cannot_authorize_formal_evidence(self) -> None:
        ...
        self.assert_code(
            ...
            lambda: contract.build_reassessment_evidence(
                draft,
    ...
    def test_complete_fixed_evidence_closes_only_offline_artifact_scope(self) -> None:
        ...
        self.assertFalse(derived["preferred_offline_candidate"])
        ...
        self.assertTrue(evidence["claims"]["metadata_only_reassessment"])
        self.assertFalse(evidence["claims"]["new_model_execution"])
        self.assertFalse(evidence["claims"]["network_used"])
        self.assertFalse(evidence["model_artifact_saved"])
        self.assertFalse(evidence["tensor_payload_saved"])
    ...
    def test_raw_evidence_hash_tamper_fails_before_recomputation(self) -> None:
        evidence_payload = contract.artifact_json_bytes(self._build())
        self.assert_code(
    ...
    def test_resealed_downstream_claim_forgery_fails_recomputation(self) -> None:
        forged = copy.deepcopy(self._build())
        forged["derived_claims"]["portable_package_eligible"] = True
        forged["claims"]["portable_package_eligible"] = True
        forged.pop("report_digest")
        forged["report_digest"] = _sha256(canonical_json_bytes(forged))
        forged_payload = contract.artifact_json_bytes(forged)
        self.assert_code(
            "EVIDENCE_RECOMPUTATION_MISMATCH",
            lambda: self._validate(forged_payload),
        )
    ...
    def test_upstream_payload_and_projection_drift_fail_closed(self) -> None:
        ...
        changed_payloads["offline_package_manifest"] += b" "
        self.assert_code(
            "PAYLOAD_SHA256_MISMATCH",
            lambda: self._validate(
                evidence_payload, upstream_payloads=changed_payloads
            ),
        )

        changed_validations = copy.deepcopy(self.upstream_validations)
        changed_validations["remote_revision_origin_attestation"][
            "offline_artifact_eligible"
        ] = True
        self.assert_code(
            "UPSTREAM_VALIDATION_MISMATCH",
            lambda: self._validate(
                evidence_payload, upstream_validations=changed_validations
            ),
        )
    ...
    def test_protocol_source_payload_drift_fails_closed(self) -> None:
        ...
        self.assert_code(
            ...
            lambda: self._validate(evidence_payload, source_payloads=changed),
        )
    ...
    def test_repository_adapter_recomputes_all_canonical_upstreams(self) -> None:
        payloads = builder.load_upstream_payloads()
        validations = builder.compute_upstream_validations(payloads)
        ...
        self.assertTrue(evidence["formal_gate_passed"])
        self.assertTrue(evidence["derived_claims"]["offline_artifact_eligible"])

    def test_new_protocol_scope_has_no_model_or_network_runtime_imports(self) -> None:
